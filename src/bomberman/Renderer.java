...
public class Renderer {
	...
	public static void playAnimation(Sprite sprite) {
		...
		if (sprite.hasValidImage()) {
			playAnimation(sprite.getSpriteImages(), sprite.getPlaySpeed(), sprite.getXPosition(), sprite.getYPosition(),
					sprite.getWidth() * sprite.getScale(), sprite.getHeight() * sprite.getScale());
		} else {
			playAnimation(gc, time, sprite.getSize(), sprite.getX(), sprite.getY(), sprite.getFrames(),
					sprite.getXPosition(), sprite.getYPosition(), sprite.getWidth(), sprite.getHeight(),
					sprite.getScale(), sprite.getReversePlay(), sprite.getPlaySpeed());
		}
	}
	...
	public static void playAnimation(GraphicsContext gc, double time, int actualSize, int startingPointX,
			int startingPointY, int numberOfFrames, int x, int y, double width, double height, double scale,
			boolean reversePlay, double playSpeed) {

		double speed = playSpeed >= 0 ? playSpeed : 0.3;

		// index reporesents the index of image to be drawn from the set of images
		// representing frames of animation
		int index = findCurrentFrame(time, numberOfFrames, speed);

		// newX represents the X coardinate of image in the spritesheet image to be
		// drawn on screen
		int newSpriteSheetX = reversePlay ? startingPointX + index * actualSize : startingPointX;
		// newY represents the X coardinate of image in the spritesheet image to be
		// drawn on screen
		...
		// System.out.println("index=" + index + " newSpriteSheetX=" + newSpriteSheetX +
		// " newSpriteSheetY=" + newSpriteSheetY + " width=" + width + " height=" +
		// height + " x=" + x + " y=" + y + " width=" + width * scale + " height=" +
		// height * scale);
